...
def extract_external_resources(content: str) -> tuple:
    ...
    # Find script src
    for match in re.finditer(r'<script[^>]*\ssrc=["\']([^"\']+)["\'][^>]*>', content, re.IGNORECASE):
        src = match.group(1)
        if 'maplibre-gl' not in src and '../../dist' not in src:
            scripts.append(src)
    
    # Find stylesheet links
    for match in re.finditer(r'<link[^>]*\shref=["\']([^"\']+)["\'][^>]*\srel=["\']stylesheet["\']|<link[^>]*\srel=["\']stylesheet["\'][^>]*\shref=["\']([^"\']+)["\']', content, re.IGNORECASE):
...
def convert_js_to_ts(js_code: str) -> str:
    ...
    imports = ["import maplibregl from 'maplibre-gl';", "import 'maplibre-gl/dist/maplibre-gl.css';"]
    ...
    if "THREE" in js_code or "three" in js_code.lower():
        ...
        if "GLTFLoader" in js_code:
            imports.append("import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';")
...
def create_example_html(name: str, title: str, description: str, styles: list, body_html: str, external_scripts: list, external_styles: list) -> str:
...
{combined_styles}
    ...
    external_style_links = "\n    ".join(f'<link rel="stylesheet" href="{s}">' for s in external_styles) if external_styles else ""
    external_script_tags = "\n    ".join(f'<script src="{s}"></script>' for s in external_scripts) if external_scripts else ""
